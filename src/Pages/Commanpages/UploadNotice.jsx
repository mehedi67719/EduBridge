...
import {
  Upload, Send, Eye, Users, UserCog, GraduationCap,
...
const UploadNotice = () => {
  ...
  const roles = [
    ...
    { id: 'principal', name: 'Principal', icon: Crown, color: 'from-amber-500 to-orange-500', desc: 'Full campus analytics, all reports, teacher management' },
    { id: 'chip_instructor', name: 'Chip Instructor', icon: UserCog, color: 'from-red-500 to-rose-500', desc: 'Department oversight, instructor coordination' },
    { id: 'instructor', name: 'Instructor', icon: GraduationCap, color: 'from-blue-500 to-indigo-500', desc: 'Class management, assignment creation, grading' },
    { id: 'junior_instructor', name: 'Junior Instructor', icon: Briefcase, color: 'from-cyan-500 to-teal-500', desc: 'Assist senior instructors, lab sessions' },
    { id: 'craft_instructor', name: 'Craft Instructor', icon: Wrench, color: 'from-emerald-500 to-green-500', desc: 'Practical training, workshop management' },
    { id: 'student', name: 'Student', icon: Users, color: 'from-purple-500 to-pink-500', desc: 'View notices, assignments, results' }
  ];
    ...
    <div className="w-full">
      <div className="text-center mb-6">
        <div className="inline-flex items-center gap-2 bg-white rounded-full px-4 py-2 mb-3 shadow-md">
          ...
          <span className="text-sm font-medium text-gray-700">Admin Panel - Notice Management</span>
        </div>
        ...
        <p className="text-gray-500 text-sm mt-1">Create and manage notices for different user roles across the campus</p>
        {dbUser && (
        ...
        <div className="lg:col-span-7">
          <div className="bg-white rounded-xl shadow-lg border border-gray-100 relative">
            ...
            <div className="px-5 py-4 border-b bg-gradient-to-r from-indigo-50 to-purple-50 rounded-t-xl">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-lg font-bold text-gray-800">Create New Notice</h2>
              </div>
            </div>
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Notice Content <span className="text-red-500">*</span></label>
                <textarea
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Notice Category</label>
                    ...
                    {formData.imageUrls.map((url, idx) => (
                      <div key={idx} className="flex items-center justify-between p-2 bg-gray-50 rounded-lg">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-sm text-gray-600 truncate max-w-[200px]">{url.slice(0, 40)}...</span>
                        </div>
              ...
              <div className="flex gap-3 pt-2">
                <button onClick={handlePreview} disabled={uiState.isSubmitting} className="px-5 py-2 rounded-lg border border-gray-200 text-gray-700 font-medium hover:bg-gray-50 disabled:opacity-50 text-sm">
                ...
                <button onClick={handleSubmit} disabled={uiState.isSubmitting || uploadLoading} className="flex-1 px-5 py-2 rounded-lg bg-gradient-to-r from-indigo-500 to-purple-500 text-white font-medium hover:shadow-lg disabled:opacity-70 text-sm">
                  {uiState.isSubmitting ? <Loader2 className="w-4 h-4 animate-spin mx-auto" /> : <><Send className="w-4 h-4 inline mr-2" /> Publish Notice</>}
                </button>
              </div>
            </div>
          </div>
          ...
          {uiState.showPreview && !uiState.isSubmitting && (
            <div className="bg-white rounded-xl shadow-lg border mt-4">
              ...
              <div className="p-4">
                <div className={`p-4 rounded-lg border-l-4 ${getPriorityColor(formData.priority)} bg-white`}>
                  ...
                  <h4 className="text-lg font-bold text-gray-800 mb-2">{formData.noticeTitle || 'Notice Title'}</h4>
                  <p className="text-gray-600 text-sm mb-3">{formData.noticeContent || 'Notice content will appear here...'}</p>
                  {formData.imageUrls.length > 0 && (
                    <div className="flex gap-2 mb-3 flex-wrap">
                      {formData.imageUrls.map((url, idx) => (
                        <img key={idx} src={url} alt="preview" className="w-20 h-20 object-cover rounded-lg" />
                      ))}
                    </div>
                  )}
        ...
        <div className="lg:col-span-5">
          <div className="bg-white rounded-xl shadow-lg border border-gray-100 sticky top-24">
            <div className="px-5 py-3 border-b bg-gradient-to-r from-indigo-50 to-purple-50 rounded-t-xl">
              ...
              <p className="text-xs text-gray-500 mt-0.5">Select target audience for your notice</p>
            </div>
            <div className="p-4 space-y-2 max-h-[500px] overflow-y-auto">
              {roles.map(role => (
                ...
                >
                  <div className={`w-8 h-8 bg-gradient-to-r ${role.color} rounded-lg flex items-center justify-center flex-shrink-0`}>
