    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex items-center gap-2 bg-white rounded-full px-4 py-2 mb-4 shadow-md border border-indigo-100">
          ...
          <p className="text-gray-500 mt-2">Create and manage assignments for students</p>
          {dbUser && (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-7">
            <div className="bg-white rounded-2xl shadow-xl border border-indigo-100 relative">
              ...
              <div className="px-6 py-5 border-b bg-gradient-to-r from-indigo-50 to-purple-50 rounded-t-2xl">
                <div className="flex items-center gap-2">
                  <BookOpen className="w-5 h-5 text-indigo-500" />
                  <h2 className="text-xl font-bold text-gray-800">Create New Assignment</h2>
                </div>
                <p className="text-sm text-gray-500 mt-1">Fill in the details to publish a new assignment</p>
              </div>
              ...
              <div className="p-6 space-y-5">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Assignment Description <span className="text-red-500">*</span></label>
                  <textarea
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        disabled={uiState.isSubmitting}
                        className="w-full pl-10 pr-4 py-3 rounded-xl border border-gray-200 focus:border-indigo-400 focus:outline-none focus:ring-4 focus:ring-indigo-100 disabled:opacity-50"
                      />
                    </div>
                  </div>
                  <div>
                    ...
                    {uiState.showLinkInput ? (
                      <div className="flex gap-2">
                        <input
                          ...
                          className="flex-1 px-4 py-2 rounded-lg border border-gray-200 focus:border-indigo-400 focus:outline-none"
                        />
                        <button onClick={addReferenceLink} className="px-4 py-2 rounded-lg bg-indigo-500 text-white hover:bg-indigo-600">
                          Add
                        </button>
                        <button onClick={() => setUiState(prev => ({ ...prev, showLinkInput: false, currentLink: '' }))} className="px-4 py-2 rounded-lg border border-gray-200 hover:bg-gray-50">
                          Cancel
                        </button>
                      </div>
                    ) : (
                ...
                <div>
                  ...
                  <div className="border-2 border-dashed border-indigo-200 rounded-xl p-6 text-center hover:border-indigo-400 transition bg-indigo-50/30">
                    <input type="file" multiple onChange={handleFileChange} className="hidden" id="fileUpload" disabled={uiState.isSubmitting} />
                    <label htmlFor="fileUpload" className="cursor-pointer">
                      <Paperclip className="w-8 h-8 text-indigo-400 mx-auto mb-2" />
                ...
                <div className="flex gap-3 pt-4">
                  <button onClick={handlePreview} disabled={uiState.isSubmitting} className="px-6 py-2.5 rounded-xl border border-indigo-200 text-indigo-600 font-medium hover:bg-indigo-50 disabled:opacity-50 transition-all">
                    <Eye className="w-4 h-4 inline mr-2" /> Preview
                  </button>
                  <button onClick={handleSubmit} disabled={uiState.isSubmitting || uploadLoading} className="flex-1 px-6 py-2.5 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-500 text-white font-medium hover:shadow-lg disabled:opacity-70 transition-all">
                    {uiState.isSubmitting ? <Loader2 className="w-5 h-5 animate-spin mx-auto" /> : <><Send className="w-4 h-4 inline mr-2" /> Publish Assignment</>}
                  </button>
                </div>
              </div>
            </div>
            ...
            {uiState.showPreview && !uiState.isSubmitting && (
              <div className="bg-white rounded-2xl shadow-xl border border-indigo-100 mt-6">
                ...
                <div className="p-6">
                  <div className={`p-4 rounded-xl border-l-4 ${
                    ...
                    <p className="text-gray-600 mb-3">{formData.assignmentDescription || 'Assignment description will appear here...'}</p>
                    ...
                    <div className="flex flex-wrap gap-2 mt-3 pt-2 border-t border-gray-100">
                      {formData.selectedRoles.map(roleId => {
                        const role = roles.find(r => r.id === roleId);
                        return role && <span key={roleId} className={`px-2 py-0.5 rounded-full text-xs font-medium bg-gradient-to-r ${role.color} text-white`}>{role.name}</span>;
                      })}
                    </div>
                    {dbUser && (
          ...
          <div className="lg:col-span-5 space-y-6">
            <div className="bg-white rounded-2xl shadow-xl border border-indigo-100 sticky top-24">
              <div className="px-6 py-4 border-b bg-gradient-to-r from-indigo-50 to-purple-50 rounded-t-2xl">
                <h3 className="font-semibold text-gray-800">Guidelines</h3>
                <p className="text-xs text-gray-500 mt-1">Assignment creation tips</p>
              </div>
              <div className="p-4 space-y-3">
                <div className="flex items-start gap-3 p-3 rounded-xl bg-indigo-50">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">Provide detailed instructions and requirements</p>
                  </div>
                </div>
                <div className="flex items-start gap-3 p-3 rounded-xl bg-purple-50">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">Give students adequate time to complete</p>
                  </div>
                </div>
                <div className="flex items-start gap-3 p-3 rounded-xl bg-emerald-50">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">Add reference links and materials</p>
                  </div>
                </div>
                <div className="flex items-start gap-3 p-3 rounded-xl bg-amber-50">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">Specify total marks and grading criteria</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-xl border border-indigo-100">
              <div className="px-6 py-4 border-b bg-gradient-to-r from-indigo-50 to-purple-50 rounded-t-2xl">
                ...
                <p className="text-xs text-gray-500 mt-1">Select target audience for this assignment</p>
              </div>
              <div className="p-4 space-y-3">
                {roles.map(role => (
                  <div 
